...
import src.moco.loader as moco_loader
...
class LocalImageFolderCL(Dataset):
    """Lazy local dataset for contrastive learning over ImageFolder indices."""
    def __init__(self, base_dataset, indices, transform=None, num_classes=None):
        self.base_dataset = base_dataset
        self.indices = list(indices)
        self.transform = transform
        self.num_classes = num_classes or len(base_dataset.classes)
        self.targets = torch.tensor([base_dataset.targets[i] for i in self.indices])
        self.loader = base_dataset.loader

    def __getitem__(self, index):
        sample_index = self.indices[index]
        path, target = self.base_dataset.samples[sample_index]
        sample = self.loader(path)
...
def _split_dataset(train_dataset, class_num, client_num, non_iid_alpha, iid, batch_size, transform):
    ...
    client_indices = _split_dataset_by_proportion(train_dataset, partition_proportions, class_num, client_num, iid, batch_size)
    return [
        LocalImageFolderCL(train_dataset, indices, transform=transform, num_classes=class_num)
...
def _split_dataset_by_proportion(dataset, partition_proportions, class_num, client_num, iid, batch_size):
    ...
    for client_idcs_for_client in client_idcs:
        np.random.shuffle(client_idcs_for_client)
    return client_idcs


def _resolve_imagefolder_roots(data_path, dataset_name):
    dataset_root = Path(data_path) / dataset_name
    candidates = [
        (dataset_root / "train", dataset_root / "val"),
        (dataset_root / "train", dataset_root / "test"),
        (dataset_root / "Dataset" / "train" / "train", dataset_root / "Dataset" / "test" / "test"),
        (dataset_root / "Dataset" / "train", dataset_root / "Dataset" / "test"),
    ]
    ...
    expected = " or ".join(f"{train_root} + {test_root}" for train_root, test_root in candidates)
    raise FileNotFoundError(
        f"Could not find train/test folders for {dataset_name}. Expected one of: {expected}"
    )
...
def create_datasets_CL(data_path, dataset_name, num_clients, num_shards, iid, diri, alpha, batchsize):
    ...
    normalize_stats = transforms.Normalize(mean=[0.48422759, 0.49005176, 0.45050278],
    ...
    augmentation_aug_cifar = [
        ...
        transforms.RandomApply([moco_loader.GaussianBlur([.1, 2.])], p=0.2),
    ...
    if dataset_name == "UCML":
        num_classes = 21
        ROOT_TRAIN, ROOT_TEST = _resolve_imagefolder_roots(data_path, dataset_name)
    ...
    if iid:
        ...
        if diri:
            ...
            shard_indices = list(torch.split(sorted_indices, shard_size))
            ...
            for i in range(num_shards // num_categories):
                for j in range(0, ((num_shards // num_categories) * num_categories), (num_shards // num_categories)):
                    shard_indices_sorted.append(shard_indices[i + j])
            used_shards = len(shard_indices_sorted)
            shard_indices_sorted.extend(shard_indices[used_shards:])
                    
            # finalize local datasets by assigning shards to each client
            client_shard_groups = np.array_split(np.arange(len(shard_indices_sorted)), num_clients)
            ...
            local_datasets = [
                LocalImageFolderCL(
                    training_dataset,
                    torch.cat([shard_indices_sorted[j] for j in shard_group]).tolist(),
                    transform=train_transform,
                    num_classes=num_classes
                )
                for shard_group in client_shard_groups
            ]
    return local_datasets, test_dataset