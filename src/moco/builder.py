...
class MoCo(nn.Module):
    ...
    def _train(self, im_q, im_k, labels):
        ...
        feat_q = self.feat_after_avg_q  # capture immediately, before the hook fires again
        k = self.encoder_q(im_k)
        feat_k = self.feat_after_avg_q

        q = nn.functional.normalize(q, dim=1)
        k = nn.functional.normalize(k, dim=1)
        logits_q = self.linear(feat_q)
        logits_k = self.linear(feat_k)
